...
if __name__ == '__main__':
    ...
    print(get('http://localhost:80/users').json())  # show all
    # Rejected
    print(get('http://localhost:80/user/3'))   # show one non existing
    print(post('http://localhost:80/user/2?title=Mikhail%20Vasilevich%20Lomonosov'))   # add at the place of existing
    print(patch('http://localhost:80/user/3?title=Just%20Lomonosov'))  # modify non existing
    print(delete('http://localhost:80/user/3'))  # delete non existing
